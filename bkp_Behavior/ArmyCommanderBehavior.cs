...
namespace ArmyCommander.Behavior
{
    public partial class ArmyCommanderBehavior : CampaignBehaviorBase
    {
        ...
        public override void RegisterEvents()
        {
            ...
            // CampaignEvents.AiHourlyTickEvent.AddNonSerializedListener(this, (x, y) => { return; });
            // CampaignEvents.OnSessionLaunchedEvent.AddNonSerializedListener(this, (x) => { return; });
            // CampaignEvents.TickPartialHourlyAiEvent.AddNonSerializedListener(this, (x) => { return; });
            // CampaignEvents.OnNewGameCreatedPartialFollowUpEndEvent.AddNonSerializedListener(this, (x) => { return; });
        }
        ...
        IL_008e:
            ...
            if (targetSettlement.IsVillage)
            {
                ...
                if (mapEvent != null &&
                mapEvent.Component is RaidEventComponent raidComp &&
                raidComp.RaidDamage > 0f)
                {
                    var raidDamage = raidComp.RaidDamage;
        ...
        {
            if (mobileParty.IsMilitia || mobileParty.IsCaravan || mobileParty.IsVillager || mobileParty.IsBandit || mobileParty.IsPatrolParty || mobileParty.IsDisbanding || mobileParty.LeaderHero == null || (mobileParty.MapFaction != Clan.PlayerClan.MapFaction && mobileParty.LeaderHero.Clan?.Kingdom == null) || mobileParty.CurrentSettlement?.SiegeEvent != null)
            {
                return;
            }


            // IMPORTANT: Ignores if it is not a mercenary!
            if (mobileParty.LeaderHero?.Clan?.Kingdom == null || !(mobileParty.LeaderHero.Clan.IsUnderMercenaryService == true))
            {
                return;
            }
            ...
            if (mobileParty.Army == null && mobileParty.PartySizeRatio > 0.6f && (mobileParty.LeaderHero.Clan.Leader == mobileParty.LeaderHero || (mobileParty.LeaderHero.Clan.Leader.PartyBelongedTo == null && mobileParty.LeaderHero.Clan.WarPartyComponents != null && mobileParty.LeaderHero.Clan.WarPartyComponents.FirstOrDefault() == mobileParty.WarPartyComponent)))
            {
                ...
                flag = !mobileParty.IsCurrentlyAtSea && mobileParty.MapEvent == null && mobileParty.LeaderHero.Clan.Influence > (float)num3 && (float)mobileParty.GetNumDaysForFoodToLast() > Campaign.Current.Models.MobilePartyAIModel.NeededFoodsInDaysThresholdForSiege && mobileParty.MapFaction.FactionsAtWarWith.AnyQ((IFaction x) => x.Fiefs.Any());
                if (flag)
                {
